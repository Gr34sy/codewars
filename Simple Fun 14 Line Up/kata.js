...
function lineUp(commands) {
  const commandTab = [...commands];

  let same_direction_counter = 0;

  let turn_counter = 0;
  let same_direction = true;

  commandTab.forEach((cmd) => {
    if(cmd != 'A'){
      turn_counter++;
      if(turn_counter % 2 == 0){
        same_direction = true;
    ...
    if(same_direction && turn_counter % 2 == 0 ){
      same_direction_counter++;
    }
  });

  return same_direction_counter
}
